...
{
	bool isSecretCovering = false;
	if (!secretZoneGateList.empty())
	{
		isSecretCovering = true;
	}

	list<PlantInfo> info;

	TouchGrass::TouchGrassType tgType = (TouchGrass::TouchGrassType)gt;

	int tw = TouchGrass::GetQuadWidth(tgType);
	...
	for (int i = 0; i < numP; ++i)
	{
		...
		{
			foundGate = false;
			for (auto it = secretZoneGateList.begin(); it != secretZoneGateList.end(); ++it)
			{
				g = (*it)->GetGate();

				if ((currEdge->v0 == g->edgeA->v0 && currEdge->v1 == g->edgeA->v1) || (currEdge->v0 == g->edgeB->v0 && currEdge->v1 == g->edgeB->v1))
				{
					foundGate = true;
					break;
				}
	...
	coll->ts_grass = TouchGrassCollection::GetTileset(sess, tgType);
	coll->gType = tgType;
...
void TerrainPolygon::GenerateSecretBorderMesh()
{
	if (ts_border == NULL)
	{
		if (totalNumBorderQuads > 0)
	...
	Edge *oppositeGateEdge = NULL; //for acute angles and stuff
	...
	for (int i = 0; i < numP; ++i)
	{
		...
		oppositeGateEdge = NULL;
		for (auto it = secretZoneGateList.begin(); it != secretZoneGateList.end(); ++it)
		{
			g = (*it)->GetGate();

			if (edge->v0 == g->edgeA->v0 && edge->v1 == g->edgeA->v1)
			{
				oppositeGateEdge = g->edgeB;
				break;
			}
			else if (edge->v0 == g->edgeB->v0 && edge->v1 == g->edgeB->v1)
			{
				oppositeGateEdge = g->edgeA;
				break;
			}
		}

		if (oppositeGateEdge == NULL )
			continue;
	...
	for (int i = 0; i < numP; ++i)
	{
		...
		int currentTypeQuadsStartIndex = currentTypeQuads - borderQuads;

		Edge *oldPrev = edge->GetPrevEdge();
		Edge *oldNext = edge->GetNextEdge();

		Edge *oldPrevNext = oppositeGateEdge->edge0->edge1;
		Edge *oldNextPrev = oppositeGateEdge->edge1->edge0;

		edge->edge0 = oppositeGateEdge->edge0;
		edge->edge1 = oppositeGateEdge->edge1;

		edge->edge0->edge1 = edge;
		edge->edge1->edge0 = edge;
		...
		//	edgeQuadTesting[i].push_back(currentTypeQuads + j * 4);

			/*if(  testSecretEdge )
			{
			SetRectColor(currentTypeQuads + j * 4, Color::Transparent);
			}*/
		...
		oppositeGateEdge->edge0->edge1 = oldPrevNext;
		oppositeGateEdge->edge1->edge0 = oldNextPrev;
	}
}
...
void TerrainPolygon::Move(Vector2i move )
{
	...
	if (grassVA != NULL)
	{
		...
		{
			//quads
			grassVA[i * 4].position += Vector2f(move.x, move.y);
			grassVA[i * 4 + 1].position += Vector2f(move.x, move.y);
			grassVA[i * 4 + 2].position += Vector2f(move.x, move.y);
			grassVA[i * 4 + 3].position += Vector2f(move.x, move.y);
		}
	}
...
void TerrainPolygon::SetMaterialType(int world, int variation)
{
	...
	TerrainWorldType oldWorldType = terrainWorldType;
...
void TerrainPolygon::SetupGrass(int i, int &grassIndex )
{
	...
	for (int j = 0; j < num; ++j)
	{
		...
		grassVA[grassIndex * 4 + 1].position = topRight;
...
V2d TerrainPolygon::GetGrassCenter(int gIndex)
{
	return V2d((grassVA[gIndex * 4 + 0].position
		+ grassVA[gIndex * 4 + 1].position
		+ grassVA[gIndex * 4 + 2].position
	...
	int gType )
{
	...
	sf::Rect<double> r(GetAABB());
	r.left -= grassSize;
	r.top -= grassSize;
	r.width += grassSize * 2;
	r.height += grassSize * 2;
...
void TerrainPolygon::Draw( bool showPath, double zoomMultiple, RenderTarget *target, bool showPoints, TerrainPoint *dontShow )
{
	...
	if (depth != 1.f)
	{
		...
		if (lockToBG && drawLayer >= DrawLayer::BG_20 && drawLayer <= DrawLayer::BG_1)
		{
			center.y = 0;
	...
	else if (renderMode == RENDERMODE_MAP)
	{
		UpdatePreviewLineColors();
		...
		for (int i = 0; i < numP; ++i )
		{
			...
			if (e->secretZone != NULL && e->secretZone->data.action == Zone::UNEXPLORED)
			{
				...
				{
					SetRectColor((*it), Color::Transparent);
				}
			}
