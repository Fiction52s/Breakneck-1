...
RewindBooster::RewindBooster(ActorParams *ap)
	:Enemy(EnemyType::EN_REWINDBOOSTER, ap)
{
	SetNumActions(Count);
...
void RewindBooster::SetLevel(int lev)
{
	level = lev;
...
void RewindBooster::AddToWorldTrees()
{
	sess->activeItemTree->Insert(this);
}

bool RewindBooster::Boost()
{
	if (action == NEUTRAL)
	{
		action = BOOST;
...
bool RewindBooster::IsBoostable()
{
...
void RewindBooster::ProcessState()
{
...
void RewindBooster::UpdateSprite()
{
	int tile = 0;
	IntRect ir;
	switch (action)
...
void RewindBooster::EnemyDraw(sf::RenderTarget *target)
{
	target->draw(sprite);
}

void RewindBooster::DrawMinimap(sf::RenderTarget *target)
{
	if (!dead)
	{
