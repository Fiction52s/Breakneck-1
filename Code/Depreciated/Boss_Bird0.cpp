...
Boss_Bird0::Projectile::Projectile( Boss_Bird0 *b, bool p_large )
{
	slowMultiple = 1;
	slowCounter = 1;
	velocity = V2d( 0, 0 );
...
void Boss_Bird0::Projectile::Reset()
{
	slowMultiple = 1;
	slowCounter = 1;
	velocity = V2d( 0, 0 );
...
void Boss_Bird0::Projectile::Draw( RenderTarget *target )
{
	if( state != NO_EXIST )
	{
		target->draw( sprite );
...
void Boss_Bird0::Projectile::HandleEntrant( QuadTreeEntrant *qte )
{
	Edge *e = (Edge*)qte;
...
bool Boss_Bird0::Projectile::ResolvePhysics( 
...
void Boss_Bird0::Projectile::UpdatePrePhysics()
{
...
void Boss_Bird0::Projectile::UpdatePhysics()
{
...
void Boss_Bird0::Projectile::UpdatePostPhysics()
{	
	int f = 0;
	switch( state )
	{
	...
	gridbgVA( sf::Quads, (GRID_SIZE) * 2 * 4 )
{
	...
	//xIndexMove = 0;
	///yIndexMove = 0;
	//loop = false; //no looping on Boss_Bird0 for now
	...
	//ts = owner->GetTileset( "Boss_Bird0.png", 80, 80 );
...
void Boss_Bird0::HandleEntrant( QuadTreeEntrant *qte )
{
...
void Boss_Bird0::BulletHitTerrain( BasicBullet *b, Edge *edge, V2d &pos )
{
	b->launcher->DeactivateBullet( b );
}

void Boss_Bird0::SetupAttackMarkers()
{
	V2d trueLeft( -gridRatio, -1.0 / gridRatio );
	V2d trueRight( gridRatio, 1.0 / gridRatio );
	V2d trueDown( -gridRatio, 1.0 / gridRatio );
...
void Boss_Bird0::BulletHitPlayer(BasicBullet *b )
{
...
void Boss_Bird0::UpdateMovement()
{
	V2d trueLeft( -gridRatio, -1.0 / gridRatio );
	V2d trueRight( gridRatio, 1.0 / gridRatio );
	V2d trueDown( -gridRatio, 1.0 / gridRatio );
...
void Boss_Bird0::SetupCinemTiming()
{
	fallTiming[0] = 4;
	fallTiming[1] = 2;
	fallTiming[2] = 8;
...
void Boss_Bird0::Init()
{
	
}

bool Boss_Bird0::UpdateCinematic()
{
...
void Boss_Bird0::UpdatePrePhysics()
{
	if( cinem != NOCINEM )
	{
...
void Boss_Bird0::PhysicsResponse()
{
	if( !dead && !dying && receivedHit == NULL )
	{
		UpdateHitboxes();
		...
		//	cout << "Boss_Bird0 received damage of: " << receivedHit->damage << endl;
			/*if( !result.second )
			{
				owner->Pause( 8 );
			}
		...
		if( IHitPlayer() )
		{
		//	cout << "Boss_Bird0 just hit player for " << hitboxInfo->damage << " damage!" << endl;
		}
	}
}

void Boss_Bird0::ClearPathVA()
{
...
sf::Vector2<double> Boss_Bird0::GetLungeDir()
{
	V2d playerPos = owner->player->position;
...
void Boss_Bird0::SetRelFacePos( sf::Vector2f &pos )
{
	portrait.sprite.setPosition( pos + Vector2f( position.x, position.y ) );
}

void Boss_Bird0::UpdatePostPhysics()
{
	if( cinem != NOCINEM )
	{
		//UpdateSprite();
...
void Boss_Bird0::Draw( sf::RenderTarget *target )
{
	//if( showFace )
	{
...
void Boss_Bird0::DrawMinimap( sf::RenderTarget *target )
{
	if( !dead && !dying )
	{
...
bool Boss_Bird0::IHitPlayer(int p )
{
...
void Boss_Bird0::UpdateHitboxes()
{
...
pair<bool,bool> Boss_Bird0::PlayerHitMe( int p)
{
	Actor *player = owner->player;
...
bool Boss_Bird0::PlayerSlowingMe()
{
	Actor *player = owner->player;
...
void Boss_Bird0::DebugDraw( RenderTarget *target )
{
	if( !dead )
	{
...
void Boss_Bird0::SaveEnemyState()
{
	stored.dead = dead;
...
void Boss_Bird0::LoadEnemyState()
{
	dead = stored.dead;
...
bool Boss_Bird0::DirIsValid( sf::Vector2i &testIndex,
	sf::Vector2i &testDir )
{
...
void Boss_Bird0::HandleRayCollision( Edge *edge, double edgeQuantity, 
	double rayPortion )
{
