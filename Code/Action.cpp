...
void GrassAction::Perform()
{
	...
	for (int i = 0; i < numDiffs; ++i)
	{
		...
		gDiff.poly->SetGrassFromAction(gDiff.index, gDiff.newValue, gDiff.newType, showGrass );
	}
	performed = true;
}
void GrassAction::Undo()
{
	...
	{
		GrassDiff &gDiff = gDiffs[i];
		gDiff.poly->SetGrassFromAction(gDiff.index, gDiff.oldValue, gDiff.oldType, showGrass);
	}
	performed = false;
}
