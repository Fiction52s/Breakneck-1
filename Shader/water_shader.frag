...
{	
	...
	vec2 stuff0 = mod( pos + vec2( 0, u_slide * .1 ) * tileLimit, tileLimit ) + q1.xy;
	vec2 stuff1 = mod( pos + vec2( 0, u_slide * -.2 ) * tileLimit, tileLimit ) + q2.xy;
	vec2 stuff2 = mod( pos + vec2( u_slide * .1 + .1, 0 ) * tileLimit, tileLimit ) + q1.xy;
	vec2 stuff3 = mod( pos + vec2( u_slide * -.2 + .1, 0 ) * tileLimit, tileLimit ) + q2.xy;
